...
export default function MatchmakingPage() {
  ...
  const [processingPartnership, setProcessingPartnership] = useState<
    string | null
  >(null)
  ...
  const transformPartnership = (
    ...
    return {
      id: partnership.id,
      name: isIncoming
        ? partnership.requesterProject?.name ||
          `${partnership.requester?.firstName || "Unknown"} ${
            partnership.requester?.lastName || "User"
          }`
        : partnership.receiverProject?.name ||
          `${partnership.receiver?.firstName || "Unknown"} ${
            partnership.receiver?.lastName || "User"
          }`,
      logo: isIncoming
        ? partnership.requesterProject?.logoUrl || "/icons/default-project.png"
        : partnership.receiverProject?.logoUrl || "/icons/default-project.png",
      description:
        partnership.description ||
        "Partnership request for collaboration opportunities.",
  ...
  const transformedRequests =
    currentUserId && Array.isArray(partnerships)
      ? partnerships.map((p) => transformPartnership(p, currentUserId))
      : []
  const filteredRequests = transformedRequests.filter(
    (request) => request.requestStatus === activeTab
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        ...
        {/* Tab Navigation */}
        <div className="inline-flex items-center gap-2 border p-1 rounded-full">
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {loading ? (
            ...
            Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="bg-synqit-surface/50 backdrop-blur-sm border border-synqit-border rounded-2xl p-6 animate-pulse"
              >
            ...
            filteredRequests.map((request) => (
              <div
                key={request.id}
                className="bg-synqit-surface/50 backdrop-blur-sm border border-synqit-border rounded-2xl overflow-hidden flex flex-col relative"
              >
                ...
                <div className="relative h-20 w-full bg-white">
                  {request.logo &&
                  request.logo !== "/icons/default-project.png" ? (
                    <Image
                      src={request.logo}
                      ...
                      <div className="w-10 h-10 relative rounded-full bg-synqit-primary/20 flex items-center justify-center">
                        {request.partner?.profileImage ? (
                          ...
                          <span className="text-synqit-primary font-bold text-sm">
                            {request.partner?.firstName.charAt(0) ||
                        ...
                        <>
                          {request.status === "PENDING" ? (
                            <>
                              <button
                                onClick={() =>
                                  handleAcceptPartnership(request.id)
                                }
                              ...
                              >
                                {processingPartnership === request.id
                                  ? "Processing..."
                                  : "Accept Partnership"}
                              </button>
                              <button
                          ...
                          <Link
                            ...
                            className="px-6 py-3 bg-synqit-surface/50 hover:bg-synqit-surface border border-synqit-border text-synqit-muted-foreground hover:text-white rounded-lg transition-colors duration-200 font-medium text-sm text-center"
                          >
                            View Details
                          </Link>
                        </>
            ...
            <div className="col-span-2 bg-synqit-surface/30 backdrop-blur-sm border border-synqit-border rounded-lg p-12 text-center">
              <div className="max-w-md mx-auto">
                <div className="w-16 h-16 bg-synqit-primary/20 rounded-full flex items-center justify-center mx-auto mb-4">
                  <svg
                    className="w-8 h-8 text-synqit-primary"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
